...
First, we compute the LSD between the low quality signal and the high quality signal. This is our baseline, i.e. the score without doing anything. We will call it $LSD_{baseline} = LSD(x_{low}, x_{high})$. Then, for each version of the model "version_x", we compute the LSD between the audio generated by the model, and the high quality audio. We call it $LSD{version\_x} = LSD(x_{improved\_by\_version\_x}, x{high}). The version with the smallest distance is the better one. 
